...
/**
 * Send a WhatsApp interactive message with a single Call-to-Action (CTA) URL button.
 * Clicking the button opens the URL directly inside the mobile browser.
 */
export async function sendCtaUrlMessage(
  ...
  const {
    ...
    buttonDisplayText, buttonUrl, contextMessageId,
  } = args

  validateInteractiveBody(bodyText)
  validateInteractiveHeaderFooter(headerText, footerText)

  if (!buttonDisplayText) throw new Error('CTA URL button requires display_text.')
  if (buttonDisplayText.length > 20) {
    throw new Error('CTA URL button display_text cannot exceed 20 characters.')
  }
  if (!buttonUrl) throw new Error('CTA URL button requires a URL.')
